...
HGLRC hglrc;
GLuint base;
float click_x = 0.f, click_y = 0.f,timer=15.0f; // initial values
double t_diff = 0;
int lifes = 10, score = 0,level=2;
bool fullscreen = false, hidecursor = false, usejoystick = true, capturejoystick = true, pausegame = false, sound = true,lclick=false,reuse=false;
char txt[1000];
		...
		for (m = 0; m < 10; m++)
		{
			...
			if (click_x > b_x && click_x < b_x + 0.11f)
				if (click_y > b_y-0.11f && click_y < b_y)
					if (lclick && !pausegame)
					{
						if (grid[n][m] == match[selected][0]) { match[selected][1] = grid[n][m]; selected++; lclick = false; if (!reuse)grid[n][m] = 11; }
					}						
		}
