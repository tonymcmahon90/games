...
void Render()
{
	...
	if (timer <= 0) {
		lifes--; timer = 20.0f + (4.0f * (level - 2.0f)); selected = 0; ResetGrid();
	}
	...
	if (won)
	{
		score++; selected = 0; timer = 20.0f+(4.0f*(level-2.0f)); level++;
		ResetGrid();
	}

	if (lifes == 0) { MessageBox(hwnd, L"Game Over", L"", MB_OK); level = 2; timer = 20; lifes = 10; selected = 0; score = 0; ResetGrid(); }

	if(level==9) { MessageBox(hwnd, L"You Win", L"", MB_OK); level = 2; timer = 20; lifes = 10; selected = 0; score = 0; ResetGrid(); }
}
