...
/*_vert VV[] = {
	{XMFLOAT4(0.0f,0.5f,0.0f,1),XMFLOAT4(1,0,0,1)},
	{XMFLOAT4(0.5f,-0.5f,0.0f,1),XMFLOAT4(1,1,0,1)},
	{XMFLOAT4(-0.5f,-0.5f,0.0f,1),XMFLOAT4(1,0,1,1)},

	{XMFLOAT4(-1.0f,-1.0f,0.0f,1),XMFLOAT4(1,0,1,1)},
	{XMFLOAT4(-1.0f,0.0f,0.0f,1),XMFLOAT4(1,1,1,1)},
	{XMFLOAT4(0.0f,0.0f,0.0f,1),XMFLOAT4(0,1,0,1)},
};*/
...
void InitD3D()
{
	...
	bufferDesc.ByteWidth = sizeof(vertex)*6; // was _vert 
	...
	/*verticies[0] = {XMFLOAT3(0.0f,0.5f,0.0f),XMFLOAT4(1,0,0,1)};
	verticies[1] = { XMFLOAT3(0.5f,-0.5f,0.0f),XMFLOAT4(1,1,0,1) };
	verticies[2] = { XMFLOAT3(-0.5f,-0.5f,0.0f),XMFLOAT4(1,0,1,1) };
	verticies[3] = { XMFLOAT3(-1.0f,-1.0f,0.0f),XMFLOAT4(1,0,1,1) };
	verticies[4] = { XMFLOAT3(-1.0f,0.0f,0.0f),XMFLOAT4(1,1,1,1) };
	verticies[5] = { XMFLOAT3(0.0f,0.0f,0.0f),XMFLOAT4(0,1,0,1) };	*/
	verticies[0] = vertex(vec3(0, 0, 0), vec4(1, 0, 0, 1));
	verticies[1] = vertex(vec3(0.5, 1, 0), vec4(0, 1, 0, 1));
	verticies[2] = vertex(vec3(1, 0, 0), vec4(0, 0, 1, 1));

	verticies[3] = vertex(vec3(-1, -1, 0), vec4(1, 1, 0, 1));
	verticies[4] = vertex(vec3(-0.5, 0, 0), vec4(0, 1, 1, 1));
	verticies[5] = vertex(vec3(0, -1, 0), vec4(1, 0, 1, 1));
	vertexBuffer->Unmap();
...
void Matrix()
{
	...
	XMMATRIX proj = { XMMatrixPerspectiveFovLH(30.f / 57.29f,(float)rc.right / rc.bottom,1.0f,30.f) };
...
{		
	...
	XMMATRIX world = { XMMatrixMultiply(XMMatrixRotationX(angle),XMMatrixRotationY(angle)) };
	angle += (0.01f / 57.29f);
	pWorldMatrixEffectVariable->SetMatrix((float*)&world);
