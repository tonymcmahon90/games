...
vec3 player = { 0,2.5f,-5.f }; // player position 
float player_angle = 0,player_speed=0;
...
int WINAPI WinMain(HINSTANCE hInst, HINSTANCE hPrev, LPSTR cmd, int show)
{
	...
	SYSTEMTIME tt,nt;
	GetSystemTime(&tt);
	DWORD tick = GetTickCount(); // limit fps 
	...
	while (true)
	{
		...
		else
		{
			...
			{
				if (GetTickCount() < tick + 10) continue;
				tick = GetTickCount();
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT message, WPARAM wparam, LPARAM lparam)
{
	switch (message)
	{
		case WM_KEYDOWN:
		{
			if (LOWORD(wparam) == 'W') { player_speed=0.06f; return 0; } // need proportional to time passed 
			if (LOWORD(wparam) == 'S') { player_speed=-0.06f; return 0; }
			if (LOWORD(wparam) == 'A') { player_angle+=0.03f; return 0; }
			if (LOWORD(wparam) == 'D') { player_angle-=0.03f; return 0; }
			return 0;
		}
		case WM_KEYUP:
		{
			if (LOWORD(wparam) == 'W' || LOWORD(wparam) == 'S') { player_speed = 0; return 0; }
...
{		
	...
	XMMATRIX proj = { XMMatrixPerspectiveFovLH(30.f / 57.29f,(float)rc.right / rc.bottom,1.0f,250.f) };
