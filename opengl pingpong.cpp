...
double t_diff = 0, difficulty = 8.0,skill=1.0;
int score = 0, c_score = 0;
bool hide_cursor = false, color = false;
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_MOUSEMOVE:
	{
		static int x_diff, y_diff, x_prev, y_prev;
		if (x_prev==0) { x_prev = GET_X_LPARAM(lParam); y_prev = GET_Y_LPARAM(lParam); }
		x_diff = GET_X_LPARAM(lParam) - x_prev;
		y_diff = GET_Y_LPARAM(lParam) - y_prev;
		x_prev = GET_X_LPARAM(lParam);
		y_prev = GET_Y_LPARAM(lParam);

		bat_x += x_diff*t_diff/difficulty; bat_x = min(-0.1f,max(-1.f, bat_x));
		bat_y -= y_diff*t_diff/difficulty; bat_y = min(0.98f,max(-0.73f, bat_y));
	}
	case WM_KEYDOWN:
		switch (wParam)
		{
...
void Render()
{
	...
	static double ball_x_direction=ball_speed, ball_y_direction=ball_speed;
