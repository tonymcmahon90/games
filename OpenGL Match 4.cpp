...
HGLRC hglrc;
GLuint base;
float click_x = 0.f, click_y = 0.f,timer=15.0f,timer2=0.0f; // initial values
double t_diff = 0;
int lifes = 10, score = 0,level=2;
bool fullscreen = false, hidecursor = false, usejoystick = true, capturejoystick = true, pausegame = false, sound = true,lclick=false,reuse=false,removegrid=true,randomremovetime=true;
char txt[1000];
int grid[10][10],match[10][2],selected=0;
RECT windowrect;
...
void Render()
{
	...
	if (!pausegame) { timer -= t_diff; timer2 += t_diff; }
		...
		for (m = 0; m < 10; m++)
		{
			...
			if (click_x > b_x && click_x < b_x + 0.11f)
				if (click_y > b_y-0.11f && click_y < b_y)
					if (lclick && !pausegame)
					{
						if (grid[n][m] == match[selected][0]) { 
							match[selected][1] = grid[n][m]; selected++; lclick = false; if (!reuse)grid[n][m] = 11; 
							if (removegrid)grid[rand() % 10][rand() % 10] = 11;
						}
					}						
		}

	if (randomremovetime && timer2 > 1.0f) { grid[rand() % 10][rand() % 10] = 11; timer2 = 0.0f; } // randomly remove every second
	...
	for (n = 0; n < 10; n++)
		for (m = 0; m < 10; m++)
		{
			...
			if (click_x > b_x-0.05f && click_x < b_x + 0.15f) // was harder just show 1 box b_x , b_x+0.1f, now show upto 4 boxes
				if (click_y > b_y - 0.15f && click_y < b_y+0.05f)
				{
	...
	if (timer <= 0) {
		lifes--; timer = 15.0f + (3.0f * (level - 2.0f)); selected = 0; ResetGrid();
	}
	...
	if (won)
	{
		score+=(int)timer; selected = 0; timer = 15.0f+(3.0f*(level-2.0f)); level++;
		ResetGrid();
	}

	if (lifes == 0) { MessageBox(hwnd, L"Game Over", L"", MB_OK); level = 2; timer = 15; lifes = 10; selected = 0; score = 0; ResetGrid(); }

	if(level==11) { MessageBox(hwnd, L"You Win", L"", MB_OK); level = 2; timer = 15; lifes = 10; selected = 0; score = 0; ResetGrid(); }
}
